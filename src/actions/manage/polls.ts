...
async function computeResults(payload: Payload, pollId: string): Promise<PollResults> {
  const [questionsRes, optionsRes, votesRes] = await Promise.all([
    payload.find({ collection: 'poll-questions', where: { poll: { equals: pollId } }, sort: 'order', limit: 500, depth: 0, overrideAccess: true }),
    payload.find({ collection: 'poll-options', where: {}, limit: 2000, depth: 1, overrideAccess: true }),
    payload.find({ collection: 'poll-votes', where: { poll: { equals: pollId } }, limit: 100000, depth: 0, overrideAccess: true }),
  ])

  const relId = (v: unknown): string | null => (v == null ? null : typeof v === 'object' ? String((v as { id: unknown }).id) : String(v))

  // distinct participants by user || sessionToken
  ...
  const questions: QuestionResult[] = questionsRes.docs.map((q) => {
    ...
    const optionResults = opts.map((o) => {
    ...
    return { id: qid, text: qq.text, type: qq.type, options: optionResults, textAnswers, scale, answerCount: qVotes.length }
  })

  return { participantCount: identities.size, questions }
}

export async function getPollResults(slug: string, pollId: string): Promise<{ error: string } | { results: PollResults }> {
  const ctx = await getProjectManagerContext(slug)
  if (!ctx) return { error: 'Nicht berechtigt.' }
  const payload = await getPayload({ config })
