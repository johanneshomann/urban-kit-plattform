...
export function ForumThreadView({ slug, locale, threadId, comments, locked, canParticipate }: { slug: string; locale: string; threadId: string; comments: ForumCommentItem[]; locked: boolean; canParticipate: boolean }) {
  ...
  return (
    <section className="mt-8">
      <h2 className="text-display font-bold mb-4" style={{ color: 'var(--project-accent)' }}>Antworten ({comments.length})</h2>

      <div className="flex flex-col gap-3 mb-5">
        {comments.length === 0 ? (
          <p className="text-text" style={{ color: 'var(--project-ink)' }}>Noch keine Antworten.</p>
        ) : comments.map((c) => (
