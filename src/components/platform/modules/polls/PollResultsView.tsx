      ...
      {results.questions.map((q) => (
        <div key={q.id}>
          ...
          {q.type === 'scale' && q.scale && (
            <div>
              <p className="text-small mb-1" style={{ color: 'var(--project-ink)' }}>Durchschnitt: <strong>{q.scale.average.toFixed(2)}</strong> ({q.scale.count} Stimmen)</p>
          ...
          {q.type === 'text' && (
            q.textAnswers.length === 0
              ? <p className="text-small" style={{ color: 'var(--project-ink)' }}>Keine Antworten.</p>
