...
export function CalendarManager({ slug, locale, events }: { slug: string; locale: string; events: EventItem[] }) {
      ...
      <div className={`${card} px-4 py-3 flex items-center justify-between gap-3`} style={{ ...cardStyle, opacity: muted ? 0.6 : 1 }}>
        <div className="min-w-0">
          ...
          <p className="text-small flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5" style={{ color: 'var(--project-dark)', opacity: 0.55 }}>
            ...
            <span className="flex items-center gap-1"><VI className="w-3.5 h-3.5" />{vm?.label ?? e.visibility}</span>
          </p>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <button type="button" onClick={() => openEdit(e)} disabled={pending} title="Bearbeiten" className="p-2 rounded-lg disabled:opacity-40" style={{ color: 'var(--project-dark)' }}><Pencil className="w-4 h-4" /></button>
          {confirmDelete === e.id ? (
            <>
              <button type="button" onClick={() => run(() => deleteProjectEvent(slug, locale, e.id), () => setConfirmDelete(null))} disabled={pending} className="px-3 py-1.5 rounded-lg text-small font-semibold disabled:opacity-40" style={{ background: '#b91c1c', color: 'white' }}>Löschen</button>
            ...
            <button type="button" onClick={() => setConfirmDelete(e.id)} disabled={pending} title="Löschen" className="p-2 rounded-lg disabled:opacity-40" style={{ color: '#b91c1c' }}><Trash2 className="w-4 h-4" /></button>
          )}
        </div>
      </div>
    )
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="text-text mb-6" style={{ color: 'var(--project-dark)', opacity: 0.65 }}>Termine und Veranstaltungen verwalten.</p>
      ...
      {editing && (
        <div className={`${card} p-5 mb-6`} style={cardStyle}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-small font-bold uppercase tracking-widest" style={{ color: 'var(--project-dark)', opacity: 0.5 }}>{editing.id ? 'Termin bearbeiten' : 'Neuer Termin'}</h2>
          ...
          <div className="flex flex-col gap-3">
            <input type="text" value={f.title} onChange={(e) => set('title', e.target.value)} placeholder="Titel …" className={`${inputCls} w-full`} style={inputStyle} />
            ...
            <div className="grid sm:grid-cols-3 gap-2">
              <input type="text" value={f.location ?? ''} onChange={(e) => set('location', e.target.value)} placeholder="Ort (optional)" className={inputCls} style={inputStyle} />
              <input type="text" value={f.category ?? ''} onChange={(e) => set('category', e.target.value)} placeholder="Kategorie (optional)" className={inputCls} style={inputStyle} />
            ...
            <div>
              <textarea value={f.body ?? ''} onChange={(e) => set('body', e.target.value)} rows={5} placeholder="Beschreibung (Markdown, optional)" className={`${inputCls} w-full font-mono`} style={inputStyle} />
            </div>
            <div>
              <button type="button" onClick={save} disabled={pending || !f.title.trim() || !f.startDate} className="flex items-center gap-1.5 px-5 py-2.5 rounded-lg text-cta font-semibold transition-opacity disabled:opacity-40" style={{ background: 'var(--project-dark)', color: 'var(--project-white)' }}>
                {editing.id ? <Pencil className="w-4 h-4" /> : <Plus className="w-4 h-4" />} {editing.id ? 'Speichern' : 'Termin erstellen'}
              </button>
            </div>
          </div>
        </div>
      ...
      <h2 className="text-small font-bold uppercase tracking-widest mt-2 mb-3" style={{ color: 'var(--project-dark)', opacity: 0.45 }}>Anstehend ({upcoming.length})</h2>
      {upcoming.length === 0 ? <p className="text-text py-4 text-center" style={{ color: 'var(--project-dark)', opacity: 0.4 }}>Keine anstehenden Termine.</p>
        : <div className="flex flex-col gap-2">{upcoming.map((e) => <Row key={e.id} e={e} />)}</div>}

      {past.length > 0 && (
        <>
          <h2 className="text-small font-bold uppercase tracking-widest mt-8 mb-3" style={{ color: 'var(--project-dark)', opacity: 0.45 }}>Vergangen ({past.length})</h2>
