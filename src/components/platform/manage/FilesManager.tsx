...
export function FilesManager({ slug, locale, folders, files }: { slug: string; locale: string; folders: FolderItem[]; files: FileItem[] }) {
      ...
      <div className="flex items-center gap-3 rounded-lg border px-3 py-2" style={cardStyle}>
        ...
        {confirm === `f-${f.id}` ? (
          <span className="flex items-center gap-1">
            <button type="button" onClick={() => run(() => deleteFile(slug, locale, f.id), () => setConfirm(null))} disabled={pending} className="px-2 py-1 rounded text-small font-semibold" style={{ background: '#b91c1c', color: 'white' }}>Löschen</button>
          ...
          <button type="button" onClick={() => setConfirm(`f-${f.id}`)} disabled={pending} title="Löschen" className="p-2 rounded-lg shrink-0" style={{ color: '#b91c1c' }}><Trash2 className="w-4 h-4" /></button>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-title font-bold leading-tight mb-1" style={{ color: 'var(--project-dark)' }}>Dateien</h1>
      <p className="text-text mb-6" style={{ color: 'var(--project-dark)', opacity: 0.65 }}>Dokumente hochladen und in Ordnern organisieren. Öffentliche Dateien erscheinen auf der Projektseite.</p>
      ...
      <div className="rounded-xl border p-4 mb-6 flex flex-wrap items-end gap-3" style={cardStyle}>
        <div>
          <label className="block text-small mb-1" style={{ color: 'var(--project-dark)', opacity: 0.6 }}>Ordner</label>
        ...
        <button type="button" onClick={() => setShowFolderForm((s) => !s)} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-cta font-medium border" style={{ color: 'var(--project-dark)', borderColor: 'color-mix(in srgb, var(--project-mid) 30%, transparent)' }}>
          {showFolderForm ? <X className="w-4 h-4" /> : <FolderPlus className="w-4 h-4" />} Ordner
        </button>
      ...
      {showFolderForm && (
        <div className="rounded-xl border p-4 mb-6 flex flex-wrap items-end gap-3" style={cardStyle}>
          <div className="flex-1 min-w-40">
            <label className="block text-small mb-1" style={{ color: 'var(--project-dark)', opacity: 0.6 }}>Ordnername</label>
            <input type="text" value={folderName} onChange={(e) => setFolderName(e.target.value)} placeholder="z. B. Protokolle" className="w-full px-3 py-2 rounded-lg border text-text outline-none" style={inputStyle} />
          </div>
          <VisSelect value={folderVis} onChange={setFolderVis} disabled={pending} />
          <button type="button" onClick={() => run(() => createFolder(slug, locale, { name: folderName, visibility: folderVis }), () => { setFolderName(''); setShowFolderForm(false) })} disabled={pending || !folderName.trim()} className="px-4 py-2 rounded-lg text-cta font-semibold disabled:opacity-40" style={{ background: 'var(--project-dark)', color: 'var(--project-white)' }}>Erstellen</button>
        </div>
      )}

      {folders.length === 0 && files.length === 0 && (
        <p className="text-text py-10 text-center" style={{ color: 'var(--project-dark)', opacity: 0.4 }}>Noch keine Dateien.</p>
      )}
      ...
      <div className="flex flex-col gap-5">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              {confirm === `fo-${folder.id}` ? (
                <span className="flex items-center gap-1 ml-auto">
                  <button type="button" onClick={() => run(() => deleteFolder(slug, locale, folder.id), () => setConfirm(null))} disabled={pending} className="px-2 py-1 rounded text-small font-semibold" style={{ background: '#b91c1c', color: 'white' }}>Ordner + Dateien löschen</button>
                  <button type="button" onClick={() => setConfirm(null)} className="px-1.5 py-1 rounded text-small" style={{ color: 'var(--project-dark)', opacity: 0.6 }}>×</button>
                </span>
              ) : (
                <button type="button" onClick={() => setConfirm(`fo-${folder.id}`)} disabled={pending} title="Ordner löschen" className="p-1.5 rounded ml-auto" style={{ color: '#b91c1c' }}><Trash2 className="w-4 h-4" /></button>
              )}
            </div>
            <div className="flex flex-col gap-2 pl-7">
              {files.filter((f) => f.folderId === folder.id).map((f) => <FileRow key={f.id} f={f} />)}
              {files.filter((f) => f.folderId === folder.id).length === 0 && <p className="text-small" style={{ color: 'var(--project-dark)', opacity: 0.4 }}>Leer.</p>}
            </div>
          </div>
        ))}
        ...
        {looseFiles.length > 0 && (
          <div>
            <h2 className="text-small font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--project-dark)', opacity: 0.45 }}>Ohne Ordner</h2>
