  ...
  return (
    <section className="max-w-2xl">
      <h1 className="flex items-center gap-2 text-display font-bold mb-1" style={{ color: 'var(--plattform-ink)' }}>
        <MessageCircle className="w-5 h-5" /> Chat-Räume
      </h1>
      <p className="text-text mb-5" style={{ color: 'var(--plattform-ink)', opacity: 0.7 }}>
        Räume sind für alle aktiven Projektmitglieder sichtbar. Neue Mitglieder werden automatisch hinzugefügt.
      </p>

      <div className="flex gap-2 mb-6">
        <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Neuer Raumname (z. B. Allgemein)"
      ...
      <div className="flex flex-col gap-2">
        {rooms.length === 0 && <p className="text-text text-gray-400">Noch keine Räume.</p>}
        {rooms.map((r) => (
          <div key={r.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border" style={{ borderColor: '#e5e7eb' }}>
            {editing?.id === r.id ? (
              ...
              <>
                ...
                <span className="text-small text-gray-400">{r.memberCount} Mitglieder</span>
                <button type="button" onClick={() => setEditing({ id: r.id, name: r.name })} title="Umbenennen" className="p-1.5 rounded hover:bg-gray-100"><Pencil className="w-4 h-4" /></button>
                <button type="button" disabled={pending} onClick={() => { if (confirm(`Raum „${r.name}“ löschen?`)) run(() => deleteRoom(r.id)) }} title="Löschen" className="p-1.5 rounded hover:bg-gray-100" style={{ color: '#b91c1c' }}><Trash2 className="w-4 h-4" /></button>
              </>
            )}
          </div>
        ))}
