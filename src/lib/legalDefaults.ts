...
export function barrierefreiheitDefault(locale: string, cityName: string) {
  ...
  if (locale === 'en') {
    return doc([
      ...
      p(txt('The publicly accessible portal – home page, area pages, project overview and archive, project pages, news, events, poll results and all editorially maintained content pages – is compliant with BITV 2.0 (WCAG 2.1, conformance level AA). The exceptions listed below concern exclusively the protected workspace, which is available only to signed-in members of a participation project. Overall, the platform is therefore partially compliant. This assessment is based on a self-evaluation carried out on [add date of self-evaluation].')),
      h('h3', 'Non-accessible content'),
      p(txt('The content listed below within the members-only workspace is not fully accessible:')),
      ul([
        ...
        [b('Content created by participants in the workspace. '), txt('Images, uploaded documents (e.g. PDF) and texts that project members share within the workspace are not checked for accessibility by us. Alternative texts for images are optional for authors. Reason: third-party content that is neither financed, developed nor controlled by us (Section 4 BITV 2.0).')],
  ...
  return doc([
    ...
    p(txt('Das öffentlich zugängliche Portal – Startseite, Bereichsseiten, Projektübersicht und -archiv, Projektseiten, Neuigkeiten, Termine, Umfrage-Ergebnisse sowie alle redaktionell gepflegten Inhaltsseiten – ist mit der BITV 2.0 (WCAG 2.1, Konformitätsstufe AA) vereinbar. Die nachfolgend aufgeführten Ausnahmen betreffen ausschließlich den geschützten Arbeitsbereich, der nur angemeldeten Mitgliedern eines Beteiligungsprojekts zugänglich ist. Insgesamt ist die Plattform daher teilweise vereinbar. Grundlage dieser Einschätzung ist eine am [Datum der Selbstbewertung ergänzen] durchgeführte Selbstbewertung.')),
    h('h3', 'Nicht barrierefreie Inhalte'),
    p(txt('Die nachstehend aufgeführten Inhalte im Arbeitsbereich für Projektmitglieder sind nicht vollständig barrierefrei:')),
    ul([
      ...
      [b('Von Teilnehmenden erstellte Inhalte im Arbeitsbereich. '), txt('Bilder, hochgeladene Dokumente (z. B. PDF) und Texte, die Projektmitglieder innerhalb des Arbeitsbereichs teilen, werden von uns nicht auf Barrierefreiheit geprüft. Alternativtexte für Bilder sind für Autor:innen optional. Begründung: Inhalte Dritter, die weder von uns finanziert noch entwickelt werden noch unserer Kontrolle unterliegen (§ 4 BITV 2.0).')],
...
export const datenschutzDe = doc([
  ...
  p(txt('Von Ihnen hochgeladene Dateien und Bilder werden direkt auf dem Server der Plattform gespeichert: [Hosting-Anbieter bzw. Eigenbetrieb, Ort/Land ergänzen]. Soweit ein externer Hosting-Anbieter eingesetzt wird, besteht mit diesem ein Auftragsverarbeitungsvertrag (Art. 28 DSGVO).')),
...
export const datenschutzEn = doc([
  ...
  p(txt('Files and images you upload are stored directly on the platform’s server: [add hosting provider or self-hosted setup, city/country]. Where an external hosting provider is used, a data processing agreement (Art. 28 GDPR) is in place.')),
